...
from yolox.tracker.fast_tracker import FastTracker
...
class MOTEvaluator:
    ...
    def __init__(
        self, args, dataloader, img_size, confthre, nmsthre, num_classes, config=None):
        """
        Args:
        ...
        self.args = args
        self.config: Dict[str, Any] = {**DEFAULT_CFG, **(config or {})}
        self._validate_cfg()

    def _validate_cfg(self):
        assert 0.0 <= self.config["track_thresh"] <= 1.0
        assert 0.0 <= self.config["match_thresh"] <= 1.0
        assert 0.0 <= self.config["init_iou_suppress"] <= 1.0
        assert self.config["track_buffer"] >= 0
        assert self.config["min_box_area"] >= 0
    def evaluate(
        self,
        model,
        distributed=False,
